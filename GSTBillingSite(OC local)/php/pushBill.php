...
elseif($_SESSION['isUsrLoggedIn'] == "true"){
	if($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		if($action == "add" && $usrPerm['billC'] == 1){
					...
					if($billResult['status'] == "true"){
						...
						if($result && mysqli_num_rows($result) > 0){
							...
							if($customerResult['true']=="true"){
								...
								log_msg("Bill item added successfully.|18|69|2");
								$flag = 1;
							}
						}
						else{
							$_SESSION['internalMsg'] = "Oops! Something went wrong !!";
							log_msg("Bill Items add failed.|18|75|4");
						}
					}
				}
				else{
					$_SESSION['internalMsg'] = "Oops! Something went wrong !!";
					log_msg("Bill Creation failed.|18|81|3");
				}
				mysqli_next_result($conn);				
			}
			...
			if($usrInfo['biltyCust'] == 0){
				...
				if($result && mysqli_num_rows($result) > 0){
					...
					if($billResult['status'] == "true"){
						...
						for($i=0; $i < count($_POST['pdtid']); $i++){
							...
							if($result && mysqli_num_rows($result) > 0){
								...
								if($customerResult['true']=="true"){
									$_SESSION['internalMsg'] = "Bill created successfully !!";
									log_msg("Bill item added successfully.|18|125|2");
									$flag = 1;
								}
							}
							else{
								$_SESSION['internalMsg'] = "Oops! Something went wrong !!";
								log_msg("Bill Items add failed.|18|131|4");
							}
							mysqli_next_result($conn);
						}
					}
					...
					$_SESSION['internalMsg'] = "Oops! Something went wrong !!";
					log_msg("Bill Creation failed.|18|139|3");
				}
				mysqli_next_result($conn);
			}		
		...
		elseif($action == "edit" && $usrPerm['billE'] == 1){
				...
				if($result && mysqli_num_rows($result) > 0){
					...
					if($billResult['status'] == "true"){
						...
						if($result && mysqli_num_rows($result) > 0){
							...
							if($customerResult['true']=="true"){
								$_SESSION['internalMsg'] = "Bill created successfully !!";
									log_msg("Bill Item Edited successfully.|18|180|2");
								$flag = 1;
							}
						}
						else{
							$_SESSION['internalMsg'] = "Oops! Something went wrong !!";
								log_msg("Bill Items Edit failed.|18|186|4");
						}
					}
				}
				else{
					$_SESSION['internalMsg'] = "Oops! Something went wrong !!";
					log_msg("Bill Edit failed.|18|192|3");
				}
				mysqli_next_result($conn);				
			}
			...
			if($usrInfo['biltyCust'] == 0){
				...
				if($result && mysqli_num_rows($result) > 0){
					...
					if($billResult['status'] == "true"){
						...
						for($i=0; $i < count($_POST['pdtid']); $i++){
							...
							if($result && mysqli_num_rows($result) > 0){
								...
								if($customerResult['true']=="true"){
									$_SESSION['internalMsg'] = "Bill created successfully !!";
									log_msg("Bill Item Edited successfully.|18|237|2");
									$flag = 1;
								}
							}
							else{
								$_SESSION['internalMsg'] = "Oops! Something went wrong !!";
								log_msg("Bill Items Edit failed.|18|243|4");
							}
							mysqli_next_result($conn);
						}
					}
					...
					$_SESSION['internalMsg'] = "Oops! Something went wrong !!";
					log_msg("Bill Edit failed.|18|251|3");
				}
				mysqli_next_result($conn);
			}			
		}
			...
			log_msg("DOM Manipulation Detected.|18|258|5");
		}
		...
		$country = test_input($_POST['country']);		
			...
			if($result && mysqli_num_rows($result) > 0){
				...
				if($sAdrsResult['status']=="true"){
					$_SESSION['internalMsg'] = "Bill created successfully !!";
					log_msg("Shipping Added successfully.|18|275|2");
				}
			}
			else{
				$_SESSION['internalMsg'] = "Shipping Address upload Failed !!";
					log_msg("Shipping Added Failed.|18|280|3");
			}
			mysqli_close($conn);
		}
		...
		log_msg("Incorrect URL Access.|18|287|4");
		send_dashboard("Incorrect URL Access.");
	}
}
else{
	log_msg("Unauthrised Access.|18|292|3");
	send_home("Please, Log In !!");
	}
?>